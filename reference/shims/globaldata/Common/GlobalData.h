...
public:
	...
	// BFME GlobalData carries 0x1C more bytes before m_drawSkyBox than ZH:
	// retail doSkyBoxSet @0x79E490 stores the converted Real to
	// TheWritableGlobalData+0x180 where our layout puts the field at +0x164.
	// Only the cumulative offset is proven, not which earlier field grew, so
	// the padding sits immediately in front of the field that pins it.
	//
	// IMPORTANT: this shim is deliberately TU-scoped and is NOT a full BFME
	// GlobalData reconstruction. Opting W3DShadow.cpp in proves why: its
	// already-matched W3DShadowManager::queryShadowStatus reads fields at our
	// +0x4C8..+0x4E8 and retail agrees with those offsets UNSHIFTED, so the
	// 0x1C added below must be balanced by 0x1C removed somewhere between
	// m_drawSkyBox and +0x4C8. Add a TU here only after checking that none of
	// its matched rows read GlobalData in the ranges this shim moves.
	char _bfme_pad_pre_drawSkyBox[0x1C];
	Real m_drawSkyBox;
	Real m_skyBoxScale;
	Real m_cameraPitch;
	Real m_cameraYaw;
  ...
  Int   m_statsInterval;       ///< if so, how many is N?
	...
	// A further 0x368 bytes appear between m_drawSkyBox and the firewall block:
	// retail writeFirewallBehavior @0x66FDF0 reads m_firewallBehavior at
	// TheGlobalData+0xB14 and writes m_firewallPortAllocationDelta at +0xB20,
	// which our layout places at +0x790 and +0x79C (a uniform +0x384 once the
	// 0x1C above is accounted for). Same caveat: cumulative offset proven,
	// position of the inserted bytes assumed.
	char _bfme_pad_pre_firewall[0x368];
